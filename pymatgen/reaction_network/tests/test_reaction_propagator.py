...
class TestKineticMonteCarloSimulator(PymatgenTest):
    def setUp(self):
        ...
        if ob:
            ...
            self.propagator = KineticMonteCarloSimulator(
                self.reaction_network, self.initial_state, self.volume
            )
    ...
    def test_get_coordination(self):
        for rr, reaction in self.propagator.reactions.items():
            if reaction.reactant_ids == ["h2o"]:
                self.assertEqual(self.propagator.get_coordination(reaction, False), 99)

        diff_prop = KineticMonteCarloSimulator(
            self.reaction_network, {"o": self.concentration, "h": self.concentration}, self.volume
        )
        for rr, reaction in diff_prop.reactions.items():
            if set(reaction.product_ids) == {"o", "h"}:
                self.assertEqual(diff_prop.get_coordination(reaction, True), 99 * 99)
            elif reaction.product_ids in [["o", "o"], ["h", "h"]]:
                self.assertEqual(diff_prop.get_coordination(reaction, True), 99 * 98 / 2)

    @unittest.skipIf(not ob, "OpenBabel not present. Skipping...")
    def test_update_state(self):
        for rr, reaction in self.propagator.reactions.items():
            if (
                reaction.reactant_ids == ["h2o"]
                and "oh-" in reaction.product_ids
                and "h+" in reaction.product_ids
            ):
